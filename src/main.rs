...
fn main() {
    ...
    let mut fonts = Glyphs::new(FONT_PATH,window.create_texture_context(),TextureSettings::new(),).unwrap();
    ...
    while let Some( event) = window.next() {
        ...
        window.draw_2d(&event, |context, g2d, device| {
            ...
            draw_text(&game_state.score_right.to_string(), &context, g2d, &mut fonts, score_right_pos);
...
fn draw_text(text: &str, context: &Context, g2d: &mut G2d,fonts: &mut Glyphs, pos: Vec2f) {
    let x_offset = fonts.width(FONT_SIZE as u32, &text).unwrap() / 2 as f64;
    text::Text::new_color(FONT_COLOR, FONT_SIZE as u32)
        .draw(text,fonts, &context.draw_state,
            context.transform.trans(pos.x - x_offset, pos.y),g2d,)
        .unwrap();
}
...
fn has_collision(bbox_a: &BBox, bbox_b: &BBox) -> bool {
    bbox_a.min.x <= bbox_b.max.x && bbox_a.max.x >= bbox_b.min.x && 
    bbox_a.min.y <= bbox_b.max.y && bbox_a.max.y >= bbox_b.min.y
}
